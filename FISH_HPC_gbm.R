...
## ---- LoadFiles
load('data/analyses.species.RData')
...
foreach (a=1:length(analyses)) %dopar% {
    ...
    for (f in 1:length(analyses[[a]]$formulas)) {
        ...
        if (speciesList[[resp]][[paste0(mod.name,'.model')]] ==0) next
        MONOTONE = assignMonotone(fish, analyses[[a]]$formulas[[f]])
        if (mod.name=='all') {fish.sub=fish
        } else {fish.sub = fish %>% filter(REGION==mod.name)}
        if (any(fish.sub[,as.character(get_response(analyses[[a]]$formulas[[f]]))]==0) & analyses[[a]]$formulas[[f]][[2]][[1]]=='log') {
        ...
        set.seed(123)
        if (length(unique(fish.sub[,resp]))<3) next
